      ...
      <div className="sticky top-24 space-y-4">
        ...
        <div className="bg-card-bg rounded-xl border border-border-main p-3 transition-colors duration-300">
          ...
          <div className="w-full bg-slate-100 rounded-full h-1">
            <div
              className="h-1 rounded-full bg-gradient-to-r from-sky-400 to-sky-600"
          ...
          <div className="p-3 border-b border-border-main bg-gradient-to-r from-page-bg to-card-bg">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-sky-100 dark:bg-sky-900/30 rounded-lg">
                <FiList className="text-sky-700 dark:text-sky-400" size={16} />
              </div>
              <div>
            ...
            <div className="mb-3">
              ...
              <div className="space-y-0.5">
                ...
                >
                  ...
                  <span className="text-text-muted">What is React?</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-text-muted">React Component</span>
                </button>
              </div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-2 mb-1">
                React Concepts ({lessons.length})
              </div>
                ...
                {lessons.map((lesson) => (
                  ...
                  >
                    <span
                      ...
                      style={{
                        backgroundColor:
                          activeSection === lesson.id ? `${lesson.color}` : 'var(--color-border-main)',
                        color: activeSection === lesson.id ? '#000' : 'var(--color-text-muted)',
                      }}
                    >
                      {lesson.id}
                    </span>
        ...
        <div className="bg-card-bg rounded-xl border border-border-main p-3 transition-colors duration-300">
          ...
          <div className="flex flex-wrap gap-1">
            {categories.slice(1).map((cat) => (
              <span
                key={cat.id}
                className="px-2 py-1 rounded-full text-[10px] font-medium text-slate-800"
                style={{
